...
export function rate(events, base = 1500, k = 32) {
    if (!(base > 100 && base < 3500 && k > 0 && k <= 100))
        throw Error("Invalid rating parameters");
    let elo = base,
        streak = 0,
        prev = null;
    const history = [];
    for (const event of events) {
        const s = event.severity,
            quality =
                s === null
                    ? 0.4
                    : Math.max(0, Math.min(10, Math.trunc(s))) / 10;
        const distance = elo - base,
            span = distance >= 0 ? 3500 - base : base - 100;
